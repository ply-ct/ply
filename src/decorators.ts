    ...
    if (typeof nameOrTarget === 'string') {
        ...
        return function (target: ClassTarget) {
            ...
            if ((target as any)[SUITE]) {
                throw new Error(
                    `@suite ${target.name} must not extend another @suite-decorated class.`
                );
            }
        ...
        if ((nameOrTarget as any)[SUITE]) {
            throw new Error(
                `@suite ${nameOrTarget.name} must not extend another @suite-decorated class.`
            );
        }
