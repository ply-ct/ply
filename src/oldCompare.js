...
OldCompare.prototype.diffLines = function(expected, actual, values, options) {
...
OldCompare.prototype.markIgnored = function(diffs, values) {
...
OldCompare.prototype.mirrorDiffs = function(diffs) {
  if (diffs) {
    var mirroredDiffs = [];
    for (let i = 0; i < diffs.length; i++) {
      var diff = diffs[i];
...
OldCompare.prototype.markLines = function(start, lines, ignored) {
  var markers = [];
  var linesIdx = 0;
  lines.forEach(line => {
    var marker = {
...
OldCompare.prototype.getMarkers = function(diffs, lines) {
  var markers = [];
  if (diffs) {
    var idx = 0;
    var lineIdx = 0;
